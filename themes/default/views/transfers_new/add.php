    ...
    $(document).ready(function () {
        ...
        $("#add_item").autocomplete({
            ...
            source: function (request, response) {
				...
				//alert($("#from_warehouse").val());
                $.ajax({
                    type: 'get',
                    url: '<?= site_url('transfersnew/suggestions'); ?>',
                    dataType: "json",
                    data: {
                        term: request.term,
...
//            }    
        ...
        function product_get_list(){
            ...
            $.ajax({
                ...
                dataType:'json',
                method:'get',
                url:'<?= site_url('transfersnew/getTransferProduct/') ?>'+arguments[0]+'/'+arguments[1],
                async:false,
                success:function(result){
                  //console.log(result);
                  setTimeout(function(){ $('#ajaxCall').hide(); }, 2000);
                    var  warehouse_form = $("#from_warehouse option:selected").text();
                    var  warehouse_to = $("#to_warehouse option:selected").text();
                    var htmlset =' <table id="qaTable2" class="table  table-bordered table-condensed table-hover table-striped dataTable">';
                        htmlset +='<thead>';
                            htmlset +='<tr>';
...
//                                            pass_variant= passvariant ;  
                                ...
                                //console.log(result[i].item.varentid);

                                var checkdesable = (result[i].item.quantity >= 1)?'':'disabled';
                               // var checkdesable = 'FALSE';  //(result[i].item.quantity<=0)?'TRUE':'FALSE';
                                htmlset+='<tr id="row_' + row_no + '" data-item-id="'+result[i].item.id+'" class="row_' +result[i].item.id+ ' sr_tr_row sr_tr_'+SR+'">';
                                
                                htmlset+='<td ><div style="display:none" id="passunites_'+result[i].item.id+'" ><select name="passunit"  class="form-control select rvariant input-xs">'+getunits+'</select> </div> <div style="display:none" id="passvariant_'+result[i].item.id+'"><select name="variant"  id="poption_'+result[i].item.id+'" onChange="getvart('+result[i].item.varentid+', this.value);" class="form-control select ">'+pass_variant+'</select></div>';
                               
                                htmlset+='<input '+checkdesable+'  class="checkbox  multi-select input-xs EachProduct" type="checkbox" onclick="myfunction(&#34;'+result[i].item.id+''+variant_id+'&#34;)" value="'+result[i].item.id+''+variant_id+'" name="val[]" id="check_box_'+result[i].item.id+''+variant_id+'" />';
                                ...
                                htmlset+='<input name="product_name[]" type="hidden" disabled="ture" id="product_name_'+result[i].item.id+''+variant_id+'" class="rname allcheck" value="'+result[i].item.name.toLowerCase()+'">';
   ...
   // Block List Function
       ...
       function myfunction(get){
            ...
            document.getElementById('rbase_qty_'+get).value=1;
            document.getElementById('quantity_'+get).value=1;
    ...
    function searchProductWarehouse(){
		var search_product_warehouse = $('#search_product_warehouse').val().toLowerCase();
    ...
    function submit_form(){
      ...
      var unit_cost = editformdata[3].value;
      
      var tax_type = $('#tax_type_'+row_id).val();
      var tax_method = $('#tax_method_'+row_id).val();
      var tax_rate = $('#tax_rate_'+row_id).val();
      ...
      document.getElementById('unit_cost_'+row_id).value=editformdata[3].value;
      document.getElementById('real_unit_cost_'+row_id).value=editformdata[3].value; 
      //console.log(varent_id);
      ...
      $('#prModaltable').hide();
       if (tax_type == 1) {
         if (tax_method == '0') {
             pr_tax_val = formatDecimal((editformdata[3].value) * parseFloat(tax_rate) / (100 + parseFloat(tax_rate)), 4);
            unit_cost = parseFloat(editformdata[3].value )- parseFloat(pr_tax_val);
         }else{
             pr_tax_val = formatDecimal(((editformdata[3].value) * parseFloat(tax_rate)) / 100, 4);
             unit_cost = parseFloat(editformdata[3].value )+ parseFloat(pr_tax_val);
         }
      }else if(tax_type == 2){
         pr_tax_val = parseFloat(tax_rate);
      }

      $('#tax_values_'+row_id).val(pr_tax_val);      
      document.getElementById('net_cost_'+row_id).value=unit_cost;

      /*12-17-2019 for quatity change product quantity*/
     // var value_tax = $('#tax_values_'+row_id).val();
      //var unit_cost = $('#net_cost_'+row_id).val();

      var total_tax_value = parseFloat(pr_tax_val) * parseFloat(quantity);
   ...
   // console.log(to_warehouse);
    ...
    $.ajax({
              ...
              url:'<?= site_url('transfersnew/getQuantity'); ?>',
              async:false,
              success:function(data){
                //console.log(data);
                if(data[0]){
 ...
 function submit_transfer(){
	...
	if (tostatus=='completed') {
		if (!confirm('Have completed status?')) {
			return false;
		}
	}  
}
    ...
    <div class="box-content">
        <div class="row">
            <div class="col-lg-12">
                ...
                echo form_open_multipart("transfersnew/add", $attrib)
                ?>
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                if($Owner || $Admin || $this->session->userdata('view_right')=='1'  ){
                                    $post = array('completed' => lang('completed'),/* 'request' => lang('Request'),*/ 'sent' => lang('sent'), 'pending' => lang('Pending'));
...
<!--                                            <br/>
                                            ...
                                            <span class="onclickbutton" id="search_prd"><i class="icon fa fa-search" data-placement="left"  title="Search Product"></i> </span>&nbsp 
                                            <span class="onclickbutton" id="prd_list"><i class="icon fa fa-list" data-placement="left"  title="Werehouse Product"></i></span> -->
                        ...
                        <div id="search_product">
                            ...
                            <div class="col-md-12">
                                ...
                                <div
                                    class="from-group"><?php echo form_submit('add_transfer', $this->lang->line("submit"), 'id="add_transfer" onclick="return submit_transfer();" class="btn btn-primary" style="padding: 6px 15px; margin:15px 0;"'); ?>
...
function product_option_model_call(product) {
		var product_options = '';
		product_options = "<div class='row'>" +
				"<div class='col-sm-12'><h2>Size</h2>";
	...
	function addProductToVarientProduct(option_id, option_name) {
		...
		var term = $(".modalvarient").find('.product_term').val() + "<?php echo $this->Settings->barcode_separator; ?>" + option_id+"<?php echo $this->Settings->barcode_separator; ?>";

		 wh = $('#from_warehouse').val();
		wh2 = $("#to_warehouse").val();
		cu = $('#slcustomer').val();
		$.ajax({
			type: "get",
			url: "<?= site_url('transfersnew/suggestions') ?>",
			data: {term: term, warehouse_id: wh,warehouse_2:wh2,option_id: option_id, customer_id: cu, option_note: note},
			dataType: "json",
			success: function (data) { 
				console.log(data);
				if (data !== null) {
