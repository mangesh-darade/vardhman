...
//        $surl = 'https://simplypos.in/posadmin/pos_error_log.php';
    ...
    public function SendSMS($mobile, $msg, $sms_template_key, $sms_header = null, $DLT_TE_ID = null) {
        ...
        if (!$sms_template_key && !$DLT_TE_ID) {
            ...
            if ($DLT_TE_ID == FALSE) {
                return '{"status":"error", "message":"SMS DLT_TE_ID not found for themplate key ' . $sms_template_key . '"}';
            }
        }
...
//style="text-align:right;"
    ...
    public function getinvoiceNo() {
        ...
        if ($gettype->financial_type == 'M') {
            ...
            $checkFinancial = $this->db->where(['financial_year' => $financial_year, 'financial_month' => date('m')])->get('sma_financial_year')->row();
            if ($this->db->affected_rows() > 0) {
                ...
                $invoice_no = 1;
                $this->db->insert('sma_financial_year', ['invoice_no' => $invoice_no, 'financial_year' => $financial_year, 'financial_month' => date('m')]);
            }

            if ($gettype->invoice_format == 'y-m-prepend-inv' || $gettype->invoice_format == 'y-m-inv') {
                $invoice_no = $invoice_no;
            } else {
                $invoice_no = date('m') . '/' . $invoice_no;
            }
