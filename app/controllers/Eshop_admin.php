...
class Eshop_admin extends MY_Controller {
     ...
     * @param type $pincode
     */
    public function actionPincode($action, $pincode) {
    ...
    public function pincodes() {
        ...
        if ($result) {
            $html = '<table class="table table-bordered"><thead><tr><th>Pincode</th><th>Outlet</th><th>Delivery Time</th><th></th></tr></thead>';
            $warehouses = $this->eshop_model->getEshopOutlets();
            foreach ($result as $pincodevalue) {
                $html .= '<tr><td>' . $pincodevalue->pincode . '</td><td>' . $warehouses[$pincodevalue->warehouse_id] . '</td><td>' . $pincodevalue->delivery_time . '</td><td><i class="fa fa-times" onclick="deletepincode(' . $pincodevalue->pincode . ')"> </i></td><tr>';
            }
            $html .= '</table>';
        } else {
