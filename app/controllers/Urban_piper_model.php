...
class Urban_piper_model  extends CI_Model{
    ...
    function action_database(){
        ...
        switch ($get_arguments[0]){
            ...
            case 'Insert':
                    $this->db->insert($get_arguments[1],$get_arguments[3]);
                    $msg = ($this->db->affected_rows()> 0 )?true:false;
                break;
            
            case 'Update':
                    $this->db->where($get_arguments[2])->update($get_arguments[1],$get_arguments[3]);
                    $msg = ($this->db->affected_rows()> 0 )?true:false;
                break;
            
            case 'Delete':
                    $this->db->where($get_arguments[2])->delete($get_arguments[1]);
                    $msg = ($this->db->affected_rows()> 0 )?true:false;
                break;
    ...
    function getStoreCategories($store_id){
