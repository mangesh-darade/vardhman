...
class Pos extends MY_Controller {
    ...
    public function index($sid = null) {
        ...
        $this->data['login_controller'] = $_SERVER["HTTP_REFERER"];
        ...
        if ($this->form_validation->run() == true && !empty($products) && !empty($data)) {
            ...
            if ($suspend) {
                ...
                $data['table_id'] = 0; //$this->input->post('table_id');
            ...
            if ($sid) {
                if ($suspended_sale = $this->pos_model->getOpenBillByID($sid)) {
                    ...
                    foreach ($inv_items as $item) {
                        ...
                        $row->qty = $item->unit_quantity;   //$item->quantity ? $item->quantity : 1;
                        $row->unit_quantity = $item->unit_quantity ? $item->unit_quantity : 1;
                        ...
                        $pr[$ri] = ['id' => $c, 'item_id' => $row_id, 'label' => $row->name . " (" . $row->code . ")",
            ...
            if ($this->uri->segment(3)) {
                ...
                $this->data['kot_tokan'] = $this->pos_model->getkottokan($this->uri->segment(3));
            }

            $this->data['order_tokan'] = $this->pos_model->getkotlog(array('kot_date' => date('Y-m-d')));
    ...
    public function getProductDataByCode($code = null, $warehouse_id = null) {
        ...
        if ($row) {
            ...
            unset($row->alert_quantity, $row->article_code, $row->brand, $row->cf1, $row->cf2, $row->cf3, $row->cf4, $row->cf5, $row->cf6, $row->divisionid, $row->file, $row->food_type_id, $row->in_eshop, $row->is_featured, $row->purchase_unit, $row->ratings_avarage, $row->ratings_count, $row->supplier3price, $row->track_quantity, $row->updated_at, $row->comments_count);
            ...
            $pr = array('id' => $c, 'item_id' => $row_id, 'label' => $row->name . " (" . $row->code . ")", 'category' => $row->category_id, 'sub_category' => $row->subcategory_id, 'row' => $row, 'combo_items' => $combo_items, 'tax_rate' => $tax_rate, 'units' => $units, 'options' => $options);
...
// redirect("pos/view/$sale_id");
    ...
    public function featuerdProducts() {
        ...
        if (!empty($products)) {
            ...
            foreach ($products as $product) {
                ...
                $prods .= "<button id=\"product-" . $category_id . $count . "\" type=\"button\" value='" . $product->code . "' title=\"" . $product->name . "\" class=\"btn-prni btn-" . $this->pos_settings->product_button_color . " product pos-tip\" data-container=\"body\"><img src=\"" . base_url() . $imgsrc . "\" alt=\"" . $product->name . "\" style='width:" . $this->Settings->twidth . "px;height:" . $this->Settings->theight . "px;' class='img-rounded' /><span>" . character_limiter($product->name, 20) . "</span></button>";
    ...
    public function getCustomerName() {
        ...
        // $sql = "SELECT id, name, phone FROM sma_companies WHERE " . $retrun . " LIKE '%" . $term . "%'  limit 50";
        $sql = "SELECT id, name, phone FROM sma_companies WHERE  IF(" . $retrun . " LIKE '%" . $term . "%'," . $retrun . " LIKE '%" . $term . "%',Replace(coalesce(" . $retrun . ",''), ' ','') LIKE '%" . str_replace(" ", "", $term) . "%' ) limit 50";
