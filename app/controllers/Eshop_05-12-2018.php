...
class Eshop extends CI_Controller {
    ...
    public function syncOfflineShop(){
        ...
        //$this->logs->write('addOfflineSaleRequest',  json_encode($cart) , $val);
        //print_r($cart);
        ...
        if(is_array($cart_data)) {
            ...
            foreach ($cart_data as $key => $cart_arr) { 
                ...
                if(!empty($order_no)){ 
                    ...
                    $product_discount   = isset($cart_arr['product_discount'])&& !empty($cart_arr['product_discount'])? $cart_arr['product_discount']:0; 
                    ...
                    $grand_total = $this->sma->formatDecimal($cart_arr['grand_total'], 2);

                    $order_total    = $item_subtotal - $order_discount; 
                    ...
                    {            
                        $rec['order_no']        = $order_no;
                        $rec['status']          = 'ERROR';
                        $rec['msg']             = 'Please check, Order cost calculation are mismatch';
                        $result['ofl_sale'][]   = (array)$rec;
                    ...
                    $order['biller_id']         = $cart_arr['biller_id']; 
                    $order['biller']            = $cart_arr['biller_name'];
                    $order['cf1']               = $cart_arr['cf1'];
                    $order['cf2']               = $cart_arr['cf2'];
                    $order['total_items']       = $cart_arr['total_item'];

                    $order['warehouse_id']    = !empty($cart_arr['default_warehouse']) ? $cart_arr['default_warehouse'] : $res->default_eshop_warehouse; 
                    $order['note']            = $note;
                    ...
                    //set Log to save
                    //$this->logs->write('addOfflineSale',  json_encode($order) , $val);
                    ...
                    if( $OrderId ){
                        ...
                        $this->site->updateReference('offapp');
                           ...
                           {
                                $product_type    = isset($_cart_item['type']) ? $_cart_item['type'] : 'standerd';
                                $hsn_code        = $_cart_item['hsn_code'];
                                $item_tax_attr   = $_cart_item['item_tax_attr'];

                                $unitPrice = $_cart_item['subtotal'] / $_cart_item['quantity'];
                                ...
                                $p_arr = array(
                                        ...
                                        'hsn_code'          =>  $hsn_code,
                                        'net_unit_price'    =>  $_cart_item['net_unit_price'],
                                        'real_unit_price'   =>  $_cart_item['unit_price'],
                                    ...
                                    if($PrdId) {
                                       ...
                                       //Add Sales Items Tax Attributes. 
                                        $taxAttr['sale_id'] = $OrderId;
                                        $taxAttr['item_id'] = $PrdId;
