...
class Products extends MY_Controller {
    ...
    function getProducts($warehouse_id = NULL) {
        ...
        if ($warehouse_id) {
            ...
            //{$this->db->dbprefix('products')}.article_code as article_code , 
            $this->datatables->select($this->db->dbprefix('products') . ".id as productid, {$this->db->dbprefix('products')}.image as image, {$this->db->dbprefix('products')}.code as code, {$this->db->dbprefix('products')}.name as name, {$this->db->dbprefix('brands')}.name as brand, {$this->db->dbprefix('categories')}.name as cname, cost as cost, price as price, COALESCE(quantity, 0) as quantity, {$this->db->dbprefix('units')}.name as unit, '' as rack, {$this->db->dbprefix('products')}.storage_type, {$this->db->dbprefix('products')}.is_featured", FALSE)
                    ->from('products')
                ...
                if ($this->input->post('attributes')) {
                    ...
                    for ($r = 0; $r <= $a; $r++) {
                        if (isset($_POST['attr_name'][$r])) {
                             $product_attributes[] = array('name' => $_POST['attr_name'][$r], 'warehouse_id' => $_POST['attr_warehouse'][$r], 'quantity' => $_POST['attr_quantity'][$r], 'price' => $_POST['attr_price'][$r],'up_price' => (isset($_POST['attr_upprice'][$r])?$_POST['attr_upprice'][$r] : NULL), 'unit_quantity' => $_POST['attr_unit_quantity'][$r],'unit_weight' => $_POST['attr_unit_weight'][$r]);
...
//exit;
    ...
    function update_price() {
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            if (isset($_FILES["userfile"])) {
                ...
                $keys = array('code', 'Product_Name', 'price', 'mrp');
                if ($this->Settings->pos_type == 'restaurant') {
                    $keys[] = 'up_price';
                }
                $keys[] = 'Variants_Name';
    ...
    public function exportcsvupdateprice() {
        ...
        if ($this->Settings->pos_type == 'restaurant') {
            $this->excel->getActiveSheet()->SetCellValue('E1', 'UP_Price');
            $this->excel->getActiveSheet()->SetCellValue('F1', 'Variants_Name');
            $this->excel->getActiveSheet()->SetCellValue('G1', 'Variants_Price');
        } else {
            $this->excel->getActiveSheet()->SetCellValue('E1', 'Variants_Name');
            $this->excel->getActiveSheet()->SetCellValue('F1', 'Variants_Price');
        }
        ...
        foreach ($getproduct as $product_val) {
            ...
            $this->excel->getActiveSheet()->SetCellValue('C' . $row, $product_val->price);
            $this->excel->getActiveSheet()->SetCellValue('D' . $row, $product_val->mrp);
            if ($this->Settings->pos_type == 'restaurant') {
                $this->excel->getActiveSheet()->SetCellValue('E' . $row, $product_val->up_price);
                $this->excel->getActiveSheet()->SetCellValue('F' . $row, $product_variants);
                $this->excel->getActiveSheet()->SetCellValue('G' . $row, $product_price);
            } else {
                $this->excel->getActiveSheet()->SetCellValue('E' . $row, $product_variants);
                $this->excel->getActiveSheet()->SetCellValue('F' . $row, $product_price);
            }
            $row++;
        }
