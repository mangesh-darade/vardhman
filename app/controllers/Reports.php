...
//        echo $warehouse_id;exit;
    ...
    function getSalesReport($pdf = NULL, $xls = NULL, $img = NULL) {
        ...
        if ($pdf || $xls || $img) {
            ...
            $this->db->select("sales.id as sale_id,sales.date as date,sales.reference_no,sales.invoice_no, sales.biller, sales.customer, sma_companies.phone, sales.grand_total, sales.total_discount, sales.paid, sales.rounding, sales.payment_status", FALSE)->from('sales')->join('sale_items', 'sale_items.sale_id=sales.id', 'left')->join('warehouses', 'warehouses.id=sales.warehouse_id', 'left');
            ...
            if (!empty($data)) {
                ...
                $this->excel->getActiveSheet()->SetCellValue('H2', lang('Varient'));
                ...
                foreach ($data as $sale_id => $salesdata) {
                    ...
                    if (!empty($data_row->items)) {
                        foreach ($data_row->items as $saleitem_id => $salesItemsData) {
                            ...
                            $this->excel->getActiveSheet()->SetCellValue('G' . $row, $sales_items_data->name);
                            $this->excel->getActiveSheet()->SetCellValue('H' . $row, $sales_items_data->variantname);
                            $this->excel->getActiveSheet()->SetCellValue('I' . $row, $sales_items_data->quantity);
                            $row++;
                        }//end foreach
                        $this->excel->getActiveSheet()->SetCellValue('J' . $rowitems, $data_row->grand_total);
                        $this->excel->getActiveSheet()->SetCellValue('K' . $rowitems, lang($data_row->total_discount));
                        $this->excel->getActiveSheet()->SetCellValue('L' . $rowitems, $data_row->paid);
                        $this->excel->getActiveSheet()->SetCellValue('M' . $rowitems, ($data_row->grand_total - $data_row->paid));
                        $this->excel->getActiveSheet()->SetCellValue('N' . $rowitems, lang($data_row->payment_status));
...
// ->group_by('sales.id');
    ...
    function getAdjustmentReport($pdf = NULL, $xls = NULL, $img = NULL) {
        ...
        if ($pdf || $xls || $img) {
            ...
            $this->db->select("DATE_FORMAT(date, '%Y-%m-%d %T') as date, reference_no, warehouses.name as wh_name, CONCAT({$this->db->dbprefix('users')}.first_name, ' ', {$this->db->dbprefix('users')}.last_name) as created_by, note, {$this->db->dbprefix('adjustments')}.id as id ", FALSE)->from('adjustments')->join('users', 'users.id=adjustments.created_by', 'left')->join('warehouses', 'warehouses.id=adjustments.warehouse_id', 'left');
