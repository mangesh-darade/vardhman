...
class Auth extends MY_Controller
{
    ...
    function profile($id = NULL)
    {
      ...
      if (!$this->ion_auth->logged_in() || !$this->ion_auth->in_group('owner') && $id != $this->session->userdata('user_id')) {
        ...
        $this->data['pos_type'] = @ucfirst($this->Settings->pos_type);
        $this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
    ...
    function login($m = NULL)
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $login_device = $this->input->post('login_device');
            ...
            if ($this->ion_auth->login($this->input->post('identity'), $this->input->post('password'), $remember)) {
                ...
                /*--------- 13-03-19 -------*/
                $sql = $this->db->select('pos-index')->where(['group_id'=>$this->session->userdata('group_id')])->get('sma_permissions')->row_array();
               
                if(is_array($sql)){
                      $this->session->set_flashdata('message', $this->ion_auth->messages());
                      
                     /* if($login_device == 'mobile') {
                          $referrer = 'storeapp/index';
                      } else {*/
                        if($sql['pos-index']=='1'){
    ...
    {   
        ...
        if($flag==0){
           redirect('pos/SendAutoEmail/sendemail_logout');
        }
   ...
   /*   if (!$code) {
        ...
        if ($user) {
            ...
            if ($this->form_validation->run() == false) {
                ...
                if ($this->_valid_csrf_nonce() === TRUE || $user->id != $this->input->post('user_id')) {
    ...
    function create_user()
    {
        ...
        $this->form_validation->set_rules('phone', lang("phone"), 'trim|is_unique[users.phone]');
        ...
        if ($this->form_validation->run() == true) {
            ...
            $additional_data = array(
                ...
                'warehouse_id' => implode(",",$this->input->post('warehouse')),
                'view_right' => $this->input->post('view_right'),
                'edit_right' => $this->input->post('edit_right'),
                'allow_discount' => $this->input->post('allow_discount'),
                'offline_mobile_app_access' => $this->input->post('offline_mobile_app_access'),
                'offline_windows_app_access' => $this->input->post('offline_windows_app_access'),
            );
            ...
            if ($this->Owner) {
                if ($id == $this->session->userdata('user_id')) {
                    ...
                    $data = array(
                        ...
                        'warehouse_id' => $this->input->post('warehouse') ? implode(",",$this->input->post('warehouse')) : NULL,
        ...
        if ($this->form_validation->run() == true) {
            ...
            if (!empty($_POST['val'])) {
                ...
                if ($this->input->post('form_action') == 'export_excel' || $this->input->post('form_action') == 'export_pdf') {
                    ...
                    $this->excel->getActiveSheet()->SetCellValue('A1', 'Users');
                    ...
                    foreach ($_POST['val'] as $id) {
                       ...
                       $this->excel->getActiveSheet()->SetCellValue('E' . $row, $user->name);
                        $this->excel->getActiveSheet()->SetCellValue('F' . $row, ($user->active==1)?'Active':'Inactive');
                        $row++;
                    }
