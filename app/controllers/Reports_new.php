...
//            ob_clean();
    ...
    public function gst_reports() {
        ...
        $this->data['warehouses'] = $this->site->getAllWarehouses();
        $this->data['billers'] = $this->site->getAllCompanies('biller');
        $this->data['supplier'] = $this->site->getAllCompanies('supplier');
    ...
    public function request_gst_report($type = NULL, $year = NULL, $month = NULL, $xls = NULL, $warehouse = NULL, $biller = NULL, $supplier = NULL, $sale_items = FALSE) {
        ...
        if ($type == "sale_gst_report") {
            ...
            $data = $this->reports_model_new->getSaleGstData($year, $month, $warehouse, $biller, $sale_items );
            ...
            if (!empty($data)) {
                ...
                foreach ($data as $row) {
                    ...
                    $rowData[$row->sale_id]['customer']         = $row->customer;
                     $rowData[$row->sale_id]['state_code']         = $row->customer;
        ...
        if($type == "purchase_gst_report") {
            ...
            $data = $this->reports_model_new->getPurchaseGstData($year, $month, $warehouse, $supplier);
    ...
    private function __sale_items_gst_report_download($rowData, $rowFooter, $sale_items) {
        ...
        if (!empty($rowData)) {
            ...
            $cellColoumnTitle = ['A' => 'Sr No',
                ...
                'U' => "Product's Tax Amount",
                'V' => 'Total Taxable Amount',
                'W' => 'Invoice Tax Amount',
                'X' => 'Total Amount',
                'Y' => 'Invoice Status',
                'Z' => 'Biller Name',
                'AA' => 'Warehouse',
            ...
            foreach ($rowData as $sale_id => $inv) {
                 ...
                 $total_taxable_amount += $inv[$cellData['V']];
                 $toal_invoice_tax_amount  += $inv[$cellData['W']];
                 $total_invoice_amount += $inv[$cellData['X']];
                ...
                foreach ($inv['items'] as $item) {
                    ...
                    $this->excel->getActiveSheet()->SetCellValue('G' . $tax_row, $item['product_name']);
                    $this->excel->getActiveSheet()->SetCellValue('H' . $tax_row, $item['product_code']);
                    $this->excel->getActiveSheet()->SetCellValue('I' . $tax_row, $item['option_name']);
                    $this->excel->getActiveSheet()->SetCellValue('J' . $tax_row, $item['quantity']);
                    $this->excel->getActiveSheet()->SetCellValue('K' . $tax_row, $item['net_unit_price']);
                    $this->excel->getActiveSheet()->SetCellValue('L' . $tax_row, $item['item_discount']);
                    $this->excel->getActiveSheet()->SetCellValue('M' . $tax_row, $item['tax_rate']);
                    $this->excel->getActiveSheet()->SetCellValue('N' . $tax_row, $item['hsn_code']);
                    ...
                    $this->excel->getActiveSheet()->SetCellValue('O' . $tax_row, $cgst_rate);
                    $this->excel->getActiveSheet()->SetCellValue('P' . $tax_row, $item['cgst']);
                    $this->excel->getActiveSheet()->SetCellValue('Q' . $tax_row, $sgst_rate);
                    $this->excel->getActiveSheet()->SetCellValue('R' . $tax_row, $item['sgst']);
                    $this->excel->getActiveSheet()->SetCellValue('S' . $tax_row, $igst_rate);
                    $this->excel->getActiveSheet()->SetCellValue('T' . $tax_row, $item['igst']);
                    $this->excel->getActiveSheet()->SetCellValue('U' . $tax_row, $item['tax_total']);
...
//                    $this->excel->getActiveSheet()->SetCellValue('V' . $tax_row, $item['total_tax_amount']);
//                    $this->excel->getActiveSheet()->SetCellValue('W' . $tax_row, $item['invoice_amount']);
//                    $this->excel->getActiveSheet()->SetCellValue('X' . $tax_row, $item['sale_status']);
//                    $this->excel->getActiveSheet()->SetCellValue('Y' . $tax_row, $item['biller']);
//                    $this->excel->getActiveSheet()->SetCellValue('Z' . $tax_row, $item['warehouse']);
//                    $this->excel->getActiveSheet()->SetCellValue('AA' . $tax_row, $item['reference_no']);
//                    $this->excel->getActiveSheet()->SetCellValue('AB' . $tax_row, $item['payment_status']);
//                    $this->excel->getActiveSheet()->SetCellValue('AC' . $tax_row, $item['payment_term']);
//                    $this->excel->getActiveSheet()->SetCellValue('AD' . $tax_row, $item['total_items']);
//                    $this->excel->getActiveSheet()->SetCellValue('AE' . $tax_row, $item['total_weight']);
             ...
             $this->excel->getActiveSheet()->SetCellValue('P' . $row , $total_cgst);
             $this->excel->getActiveSheet()->SetCellValue('R' . $row , $total_sgst);
             $this->excel->getActiveSheet()->SetCellValue('T' . $row , $total_igst);
            
            $this->excel->getActiveSheet()->SetCellValue('U' . $row , $total_tax_amount);
            
            $this->excel->getActiveSheet()->SetCellValue('V' . $row , $total_taxable_amount);
            $this->excel->getActiveSheet()->SetCellValue('W' . $row , $toal_invoice_tax_amount);
            $this->excel->getActiveSheet()->SetCellValue('X' . $row , $total_invoice_amount);
    ...
    private function __sale_gst_report_download($rowData, $rowFooter) {
        ...
        if (!empty($rowData)) {
            ...
            $cellColoumnTitle = ['A' => 'Sr No',
                ...
                'P' => "Product's  Tax Amount",
                'Q' => 'Invoice Tax Amount',
                'R' => 'Total Discount',
                'S' => 'Total Amount',
                'T' => 'Invoice Status',
                'U' => 'Biller Name',
           ...
           // $this->excel->getActiveSheet()->getStyle("A" . $row . ":Y" . $row)->getBorders()->getTop()->setBorderStyle(PHPExcel_Style_Border::BORDER_MEDIUM);
            ...
            foreach ($rowData as $invoice_no => $inv) {
               $total_tax_amount += $inv[$cellData['O']];

               $total_invoice_tax_amount += $inv[$cellData['Q']];
               $total_discount += $inv[$cellData['R']];
               $total_amount += $inv[$cellData['S']];
               ...
               $this->excel->getActiveSheet()->SetCellValue('N' . $row, $inv[$cellData['N']]);
               $this->excel->getActiveSheet()->SetCellValue('AA' . $row, $inv[$cellData['AA']]);
                $tx = 0;
                $tax_row = '';
                foreach ($inv['gst'] as $tax) {
                    ...
                    $this->excel->getActiveSheet()->SetCellValue('G' . $tax_row, $tax['tax_rate']);
                    $this->excel->getActiveSheet()->SetCellValue('H' . $tax_row, $tax['total_items']);
                    ...
                    $this->excel->getActiveSheet()->SetCellValue('I' . $tax_row, $cgst_rate);
                    $this->excel->getActiveSheet()->SetCellValue('J' . $tax_row, $tax['cgst']);
                    $this->excel->getActiveSheet()->SetCellValue('K' . $tax_row, $sgst_rate);
                    $this->excel->getActiveSheet()->SetCellValue('L' . $tax_row, $tax['sgst']);
                    $this->excel->getActiveSheet()->SetCellValue('M' . $tax_row, $igst_rate);
                    $this->excel->getActiveSheet()->SetCellValue('N' . $tax_row, $tax['igst']);
                    $this->excel->getActiveSheet()->SetCellValue('O' . $tax_row, $tax['taxable_amount']);
                    $this->excel->getActiveSheet()->SetCellValue('P' . $tax_row, $tax['tax_total']);
                ...
                $row++;
                $this->excel->getActiveSheet()->getStyle("A" . $row . ":AA" . $row)->getBorders()->getTop()->setBorderStyle(PHPExcel_Style_Border::BORDER_MEDIUM);
             ...
             $this->excel->getActiveSheet()->SetCellValue('J' . $row , $total_cgst);
             $this->excel->getActiveSheet()->SetCellValue('L' . $row , $total_sgst);
             $this->excel->getActiveSheet()->SetCellValue('N' . $row , $total_igst);
                
             
             $this->excel->getActiveSheet()->SetCellValue('O' . $row , $total_tax_amount);
             $this->excel->getActiveSheet()->SetCellValue('P' . $row , $total_product_tax_amount);
             $this->excel->getActiveSheet()->SetCellValue('Q' . $row , $total_invoice_tax_amount);
             $this->excel->getActiveSheet()->SetCellValue('R' . $row , $total_discount);
             $this->excel->getActiveSheet()->SetCellValue('S' . $row , $total_amount);
             
             $this->excel->getActiveSheet()->getStyle("A" . $row . ":AA" . $row)->getBorders()->getTop()->setBorderStyle(PHPExcel_Style_Border::BORDER_MEDIUM);

            
             
            foreach (range('B', 'AA') as $cell_4) {
                $this->excel->getActiveSheet()->getColumnDimension($cell_4)->setWidth(20);
            }//end foreach
