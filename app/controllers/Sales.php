    ...
    public function suggestions() {
        ...
        if ($rows) {
            ...
            foreach ($rows as $row)  {
                ...
                $pr[] = ['id' => ($c + $r), 'item_id' => $row_id, 'otp' => $opt ,'image' => $row->image, 'label' => $row->name . " (" . $row->code . ")", 'category' => $row->category_id, 'sub_category' => $row->subcategory_id,
...
////                $rows->item_note = $item_note;
                if ($rows) {
                    ...
                    foreach ($rows as $row) {
                        ...
                        unset($row->cost, $row->details, $row->product_details, $row->barcode_symbology, $row->supplier1price, $row->supplier2price, $row->cfsupplier3price, $row->supplier4price, $row->supplier5price, $row->supplier1, $row->supplier2, $row->supplier3, $row->supplier4, $row->supplier5, $row->supplier1_part_no, $row->supplier2_part_no, $row->supplier3_part_no, $row->supplier4_part_no, $row->supplier5_part_no);
                        unset($row->alert_quantity, $row->article_code, $row->brand, $row->cf1, $row->cf2, $row->cf3, $row->cf4, $row->cf5, $row->cf6, $row->cost, $row->divisionid, $row->file, $row->food_type_id, $row->in_eshop, $row->is_featured, $row->purchase_unit, $row->ratings_avarage, $row->ratings_count, $row->supplier3price, $row->track_quantity, $row->updated_at, $row->comments_count);
                        ...
                        $pr[] = ['id' => ($c + $r), 'item_id' => $row_id, 'image' => $row->image, 'label' => $row->name . " (" . $row->code . ")", 'category' => $row->category_id, 'sub_category' => $row->subcategory_id,
                            'row' => $row, 'combo_items' => $combo_items, 'tax_rate' => $tax_rate, 'units' => $units, 'options' => $options, 'batchs' => $batchoption, 'product_batches' => $productbatches, 'note' => ($option_note) ? $option_note : ""];
                        $r++;
                        //unset($row);
                    }
